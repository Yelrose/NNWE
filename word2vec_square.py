...
try:
    from word2vec_square_inner import train_sentence_sg, train_sentence_cbow, FAST_VERSION
except ImportError:
    # failed... fall back to plain numpy (20-80x slower training than the above)
    FAST_VERSION = -1

    def train_sentence_sg(model, sentence, alpha, work=None,neu1=None):
        """
...
class Word2Vec(utils.SaveLoad):
    ...
    def _do_train_job(self, job, alpha, inits):
        ...
        for sentence in job:
            if self.sg:
                tally += train_sentence_sg(self, sentence, alpha, work,neu1)
            else:
    ...
    def reset_weights(self):
        ...
        for i in xrange(len(self.vocab)):
            ...
            self.syn0[i] = self.syn0[i] * (self.syn0[i] > 1e-3)
        if self.hs:
    ...
    def seeded_vector(self, seed_string):
        ...
        return 2 * (once.rand(self.vector_size) - 0.5)
