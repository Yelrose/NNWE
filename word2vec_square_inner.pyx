...
cdef void our_non_neg(const int * N,float * X) nogil:
    cdef int i
    for i from 0 <= i < N[0] by 1:
        if X[i] < 1e-5:
            X[i] = 0
...
cdef void our_square(const int * N,const float * X,float * Y) nogil:
    cdef int i
    for i from 0 <= i < N[0] by 1:
        Y[i] = X[i] * X[i]
...
cdef void fast_sentence_sg_hs(
    ...
    our_square(&size,&syn0[row1],neu1)
    ...
    for i from 0 <= i < size by 1:
        syn0[row1 + i ] = syn0[row1 + i] + work[i] * neu1[i] * 2
    #our_saxpy(&size, &word_locks[word2_index], work, &ONE, &syn0[row1], &ONE)
    #our_non_neg(&size,&syn0[row1])
...
def train_sentence_sg(model, sentence, alpha, _work, _neu1):
        ...
        for i in range(sentence_len):
            ...
            for j in range(j, k):
                if j == i:
                    continue
                if hs:
                    fast_sentence_sg_hs(points[i], codes[i], codelens[i], syn0, syn1, size, indexes[j], _alpha, work,neu1, word_locks)
                if negative:
                    next_random = fast_sentence_sg_neg(negative, cum_table, cum_table_len, syn0, syn1neg, size, indexes[i], indexes[j], _alpha, work,neu1, next_random, word_locks)
